...
<%
	...
	if("reg".equals(action)){
		if(author.isExistAuthorInfo(request.getParameter("txtUserName"))){
			out.println("<script>alert('用户已经存在');location.href='author-regist.jsp';</script>");
		}else{
			auInfo.setAuthorName(request.getParameter("txtUserName"));
			auInfo.setAuthorPassword(request.getParameter("txtPassword"));
			auInfo.setAuthorEmail(request.getParameter("txtEmail"));
			author.insert(auInfo);
			out.println("<script>alert('注册成功，马上去登陆');location.href='author-login.jsp';</script>");
		}
	}
%>
<div class="context">
	<form id="Regist" name="regist" method="post" action="author-regist.jsp?action=reg" 
